...
const {
    ...
    NullLiteral:    TokenNullLiteral,
    BooleanLiteral: TokenBooleanLiteral,
    StringLiteral:  TokenStringLiteral,
    NumericLiteral: TokenNumericLiteral,
    TemplateNoSub:  TokenTemplateNoSub,
    TemplateMiddle: TokenTemplateMiddle,
    TemplateTail:   TokenTemplateTail,
    TemplateHead:   TokenTemplateHead,
...
const Unsupported                     = '{value} is not supported';
...
function getValue(value, type) {
  if (type === TokenStringLiteral || type === TokenTemplateNoSub) {
    return cook(value.slice(1, -1));
...
function TemplateLiteral(exprs, quasis, start, end) {
...
function TemplateElement(value, tail, start, end) {
...
function Literal(raw, value, start, end) {
  this.type  = 'Literal';
  this.value = value;
  this.raw   = raw;
  this.start = start;
...
Parser.prototype = {
  ...
  error(msg, token={}) {
    ...
    const value   =
      // Reads "Unexpected number" when transformed or "Unexpected token ')'"
      transform[token.type]
        ? transform[token.type]
        : `token "${token.value}"`;
    const line    = token.line;
    const column  = token.column;
    const message = replace(msg, {
      token: value,
      value: `"${token.value}"`,
    });
  ...
  parseIdentifier() {
    ...
    return new Identifier(cook(token.value), token.start, token.end);
  },

  parseTemplateElement() {
    const token     = this.nextToken();
    const type      = token.type;
    const tail      = type === TokenTemplateNoSub || type === TokenTemplateTail;
    const endSlice  =
      (type === TokenTemplateHead || type === TokenTemplateMiddle) ? -2 : -1;
    const unwrapped = cook(token.value.slice(1, endSlice));

    const value = { raw: unwrapped };

    return new TemplateElement(value, tail, token.start, token.end);
  },

  /**
   * Parses a template literal token. nextToken() should not return null here.
   */

  parseTemplate() {
    const begin = this.peek();
    const startType = begin.type;
    const expressions = [];
    const quasis = [this.parseTemplateElement()];

    let tail  = false;
    let token = null;

    if (startType === TokenTemplateNoSub) {
      return new TemplateLiteral(expressions, quasis, begin.start, begin.end);
    }

    if (this.peek() === null) {
      this.error(UnterminatedTemplate, begin);
    }
    ...
    while ((token = this.peek())) {
      ...
      if (type === TokenTemplateMiddle) {
        ...
        expressions.push(this.parseExpression());
      } else if (type === TokenTemplateTail) {
        quasis.push(this.parseTemplateElement());
        tail = true;
        break;
      } else {
    ...
    return new TemplateLiteral(expressions, quasis, begin.start, last.end);
  },

  parsePrimaryExpression() {
    const token = this.ensure(1);
    const type  = token.type;
    const value = token.value;
  ...
  parseObjectLiteral() {
    ...
    while ( ! this.match('}')) {
      ...
      if (token.value === '[') {
        ...
        this.nextToken();
        key = new Literal(token.value, getValue(token.value, token.type), token.start, token.end);
        this.expect(':');
        value = this.parseAssignmentExpression();
        end = value.end;
      } else {
    ...
    if (token !== null) {
      ...
      if (type === TokenIdentifier || value === 'let') {
        return this.parseIdentifier();
          ...
          type === TokenBooleanLiteral ||
          type === TokenNullLiteral    ||
          type === TokenTemplateNoSub
        ) {
        node = new Literal(value, getValue(value, type), token.start, token.end);
      }
