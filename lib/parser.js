...
Parser.prototype = {
  ...
  parseTemplateElement() {
    ...
    const unwrapped = token.value.slice(1, endSlice);
    // console.log(endSlice, type, `"${token.value}"`, `"${unwrapped}"`);
  ...
  parseTemplate() {
    ...
    } else if (startType !== TokenTemplateHead) {
      this.error(UnexpectedToken, begin);
    } else {
      expressions.push(this.parseExpression());

      while ((token = this.peek())) {
        if (token.type === TokenTemplateMiddle) {
          quasis.push(this.parseTemplateElement());
          expressions.push(this.parseExpression());
        } else if (token.type === TokenTemplateTail) {
          quasis.push(this.parseTemplateElement());
          tail = true;
          break;
        } else {
...
var data = "`hello${`start${'lul'}end`}wtf`";
