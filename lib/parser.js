...
const ParserPrototype = {
    ...
    init(data, opts) {
        const options = opts || {};
        const { throwSourceError=true, consumeLeast=false } = options;
    ...
    error(message, _token, expr) {
        ...
        const hasPosition = [line, column].every(i => typeof i === 'number');
        ...
        if (this.throwSourceError && hasPosition) {
            ...
            err = new Error(errorMessage + ((line && column) ? ` (${line}:${column})` : ''));
            err.name = name;
...
const ParsingFunctions = {
    ...
    parseExpressionStatement() {
        ...
        if (second && ';' === second.value) {
            ...
            this.hasMore = true;
        } else if (this.consumeLeast === false && second) {
