...
const ParsingFunctions = {
    ...
    /**
     * A convenience method. Checks should be made beforehand to ensure there is
     * actually an identifier token in the stream.
     */

    parseIdentifier() {
        const token = this.peek();
        const node = new Identifier(token.value, token.start, token.end);
...
// parse('yield')
// parse('yield = 1123', { context: { strict: true } })
// // parse('yield* = 1')
// parse('yield = yield')
