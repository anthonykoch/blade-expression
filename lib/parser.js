...
const ParsingFunctions = {
    ...
    /**
     * Parses a literal node. nextToken() should not return null here.
     */

    parseLiteral() {
        const token = this.nextToken();
        const type = token.type;
        return new Literal(token.value, token.start, token.end);
    },
    ...
    parsePrimaryExpression() {
        ...
        if ((type === TokenKeyword
            ...
            || type === TokenNumericLiteral || type === TokenStringLiteral) {
        ...
        if (token && skipSimpleOperatorCheck || lookahead && simpleop[lookahead.value]) {
            ...
            if (type === TokenIdentifier) {
                ...
                return new Identifier(token.value, token.start, token.end);
                        ...
                        || (type === TokenKeyword && Raw[token.value])) {
                this.nextToken();
                return new Literal(token.value, token.start, token.end);
            }
        }
