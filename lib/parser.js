...
function isValidSimpleAssignmentTarget_Update(node) {
	const isObject =
		ArrayExpression.name === node.type || ObjectExpression.name === node.type;
...
function isBinding(expr) {
	...
	return (
			...
			|| SpreadElement.name    === type
			|| ArrayExpression.name  === type
			|| ObjectExpression.name === type
		);
}
...
const ParsingFunctions = {
	...
	rewriteNonPattern(body) {
		for (let i = 0; i < body.length; i++) {
			...
			if (   ArrayExpression.name  === expr.type
				|| ObjectExpression.name === expr.type) {
	...
	checkSimplePrimary(token, skipOperatorCheck) {
	...
	parseYieldExpression() {
		...
		const end = argument ? argument.end : token.end;
