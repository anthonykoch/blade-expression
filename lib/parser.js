...
Parser.prototype = {
  ...
  checkSimplePrimary(token, skipSimpleOperatorCheck) {
    ...
    // if (token && (skipSimpleOperatorCheck || (lookahead && simpleop[lookahead.value]))) {
      ...
      if (type === TokenIdentifier || token.value === 'let') {
  ...
  shortcutExpression() {
    const lookahead = this.lexer.lookahead(4);
    const stashLength = this.lexer.stash.length;
    const begin = this.peek();
    let expr = null;

    if (stashLength === 1 && isSimplePrimary(begin)) {
      expr = this.checkSimplePrimary(begin, true);
      this.nextToken();
    }
    else if (
        stashLength === 3                    &&
        lookahead === null                   &&
        isSimplePrimary(this.lexer.stash[0]) &&
        simpleop[this.lexer.stash[1].value]  &&
        isSimplePrimary(this.lexer.stash[2])
      ) {
      const left = this.checkSimplePrimary(begin);
      const op = this.nextToken().value;
      const right = this.checkSimplePrimary(this.peek());
      expr = new BinaryExpression(op, left, right, left.start, right.end);
    }
...
function isSimplePrimary(token) {
  const type = token === null ? -1 : token.type;
  return (
