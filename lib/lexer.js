...
Lexer.prototype = {
  ...
  getStringLiteral() {
    ...
    const quote  = this.source.charCodeAt(start);
    let char     = quote;
    let i        = start;
    const type   = Token.StringLiteral;

    if (char !== 34 && char !== 39) {
      return null;
    }

    // += 1 to skip the beginning quote
    for (i += 1; i < this.source.length; i++) {
      char = this.source.charCodeAt(i);

      if (char === quote && this.source.charCodeAt(i - 1) !== 92) {
        break;
      } else if ((char === 10 || char === 13) && this.source.charCodeAt(i - 1) !== 92) {
        this.position = i;
        this.error('Unexpected token');
      } else if (char === 13 && this.source.charCodeAt(i + 1) === 10) {
        // \r\n
        i++;
      }
    }

    if (this.source.charCodeAt(i) !== char) {
      this.error('Unterminated string literal');
    }

    let value = this.source.substring(start, i + 1);
...
function createTokenGetter(regex, type) {
  return function () {
